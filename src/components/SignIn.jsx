...
export default function SignIn() {
  ...
  const handlePasswordLogin = async (e) => {
    ...
    try {
      const res = await axios.post("https://quickdeliverlite-backend-1.onrender.com/api/auth/login", {
        email,
        password,
      });
  ...
  const handleSendOtp = async () => {
    ...
    try {
      await axios.post("https://quickdeliverlite-backend-1.onrender.com/api/auth/send-otp", { email });
  ...
  const handleVerifyOtp = async (e) => {
    ...
    try {
      const res = await axios.post(
        "https://quickdeliverlite-backend-1.onrender.com/api/auth/verify-otp",
        { email, otp }
      );
      const { token, user } = res.data;
      localStorage.setItem("token", token);
  ...
  const handleGoogleLogin = () => {
    window.open("https://quickdeliverlite-backend-1.onrender.com/api/auth/google", "_self");
  };
