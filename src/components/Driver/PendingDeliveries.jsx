...
const PendingDeliveries = ({ onAcceptDelivery }) => {
  ...
  useEffect(() => {
    const fetchDeliveries = async () => {
      try {
        const res = await axios.get('https://quickdeliverlite-backend-1.onrender.com/api/delivery/pending');
  ...
  const handleAcceptDelivery = async (deliveryId) => {
    ...
    try {
      await axios.put(`https://quickdeliverlite-backend-1.onrender.com/api/delivery/accept/${deliveryId}`, {
        driverEmail,
      });
