...
const TrackDelivery = () => {
  ...
  const handleTrack = async (e) => {
    ...
    try {
      const response = await fetch(`https://quickdeliverlite-backend-1.onrender.com/api/delivery/track/${trackingId}`);
      if (!response.ok) {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      fetch(`https://quickdeliverlite-backend-1.onrender.com/api/delivery/track/${trackingId}`)
        .then((res) => {
