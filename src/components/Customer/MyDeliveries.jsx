...
const MyDeliveries = () => {
  ...
  useEffect(() => {
    const fetchDeliveries = async () => {
      try {
        ...
        const response = await axios.get(`https://quickdeliverlite-backend-1.onrender.com/api/delivery/customer/${userEmail}`, {
          headers: {
  ...
  const handleCancel = async (id) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.put(`https://quickdeliverlite-backend-1.onrender.com/api/delivery/cancel/${id}`, {}, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
